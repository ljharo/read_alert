    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "Vigilancia Flexible 🔓", callback_data="set_modo_flexible"
            )
        ],
    ]
...
async def manejar_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with Session() as session:
        ...
        # 2. Selección de Hora (TODO este bloque debe estar indentado)
            ...
            h, m = map(
                int, hora_elegida.split(":")
            )  # Ahora esto solo corre si hay una hora
            ...
            if context.user_data.get("temp_modo") == "fijo":
                ...
                await query.edit_message_text(
                    f"✅ Disciplina fija establecida: {hora_elegida}. El Gran Hermano vigila."
                )
                ...
                context.job_queue.run_once(
                    alarma_lectura, when=time(h, m), chat_id=user_id
                )
                await query.edit_message_text(
                    f"✅ Horario para hoy registrado: {hora_elegida}. No falles al Partido."
                )
